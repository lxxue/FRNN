...
__global__ void CountingSort2DKernel (
    ...
    int* __restrict__ sorted_points_idxs, // (N, P): sorted[n, i] = unsorted[n, idxs[i]]
    int N,
    int P,
    int G) {
...
void CountingSortCUDA(
  ...
  at::checkAllSameGPU(c, {points_t, lengths_t, grid_cell_t, grid_idx_t, grid_off_t,
      sorted_points_t, sorted_points_idxs_t});
  at::checkAllSameType(c, {grid_cell_t, grid_idx_t, grid_off_t, sorted_points_idxs_t});
  ...
  if (D == 2) {
    CountingSort2DKernel<<<blocks, threads, 0, stream>>>(
      ...
      sorted_points_idxs.contiguous().data_ptr<int>(),
      N,
      P,
      G
    );
