...
void SetupGridParams(
  ...
  params[GRID_TOTAL]= params[GRID_RES_X] * params[GRID_RES_Y] *
params[GRID_RES_Z]; params[GRID_DELTA] = 1 / cell_size; return;
}
*/

__global__ void
InsertPoints2DKernel(const float *__restrict__ points,
...
__global__ void
InsertPoints3DKernel(const float *__restrict__ points,
                     const long *__restrict__ lengths,
                     const float *__restrict__ params,
                     // TODO: add __restrict__ flag here
                     int *grid_cnt, // not sure if we can use __restrict__ here
...
template <int K> struct FindNbrsKernelFunctor {
  static void run(int blocks, int threads, int D,
...
FindNbrsCUDA(const at::Tensor points1, const at::Tensor points2,
             ...
             int K, const at::Tensor rs, const at::Tensor r2s) {
  ...
  at::checkAllSameGPU(c, {points1_t, points2_t, lengths1_t, lengths2_t,
                          pc2_grid_off_t, sorted_points1_idxs_t,
                          sorted_points2_idxs_t, params_t, rs_t, r2s_t});
