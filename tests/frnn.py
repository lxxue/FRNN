...
class ValidateFRNN:
  ...
  def compare(self):
    ...
    diff_keys_percentage = torch.sum(idxs_grid == idxs_bf).type(torch.float).item() / self.K / self.max_num_points / self.num_pcs
    dists_all_close = torch.allclose(dists_bf, dists_grid)
    return [self.fname, self.num_points, diff_keys_percentage, dists_all_close]
